...
from graph_mem.utils.config import Config, ConfigError
...
def test_config_backend_type_env(monkeypatch: pytest.MonkeyPatch) -> None:
    """GRAPHMEM_BACKEND_TYPE env var is picked up."""
    monkeypatch.setenv("GRAPHMEM_BACKEND_TYPE", "sqlite")
    config = Config()
    assert config.backend_type == "sqlite"


def test_config_backend_type_invalid_env(monkeypatch: pytest.MonkeyPatch) -> None:
    """Invalid GRAPHMEM_BACKEND_TYPE raises ConfigError."""
    monkeypatch.setenv("GRAPHMEM_BACKEND_TYPE", "neo4j")
    with pytest.raises(ConfigError, match="GRAPHMEM_BACKEND_TYPE"):
        Config()
...
def test_config_use_onnx_env_false(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("GRAPHMEM_USE_ONNX", "false")
    config = Config()
    assert config.use_onnx is False


def test_config_use_onnx_env_yes(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("GRAPHMEM_USE_ONNX", "yes")
    config = Config()
...
def test_config_cache_size_env(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("GRAPHMEM_CACHE_SIZE", "500")
    config = Config()
    assert config.cache_size == 500


def test_config_cache_size_env_invalid(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("GRAPHMEM_CACHE_SIZE", "not_a_number")
...
def test_config_embedding_model_env(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("GRAPHMEM_EMBEDDING_MODEL", "custom-model-v3")
    config = Config()
...
def test_ensure_db_dir_returns_absolute(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    config = Config(db_path=Path(".graphmem/graph.db"))
    resolved = config.ensure_db_dir()
...
def test_resolve_db_path_db_flag(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """--db flag sets GRAPHMEM_DB_PATH."""
    from graph_mem.cli.main import _resolve_db_path

    monkeypatch.delenv("GRAPHMEM_DB_PATH", raising=False)
    db_file = tmp_path / "explicit.db"
    _resolve_db_path(db_path=str(db_file))
    assert os.environ["GRAPHMEM_DB_PATH"] == str(db_file.resolve())


def test_resolve_db_path_project_dir(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """--project-dir sets GRAPHMEM_DB_PATH to <dir>/.graphmem/graph.db."""
    from graph_mem.cli.main import _resolve_db_path

    monkeypatch.delenv("GRAPHMEM_DB_PATH", raising=False)
    _resolve_db_path(project_dir=str(tmp_path))
    expected = str((tmp_path / ".graphmem" / "graph.db").resolve())
    assert os.environ["GRAPHMEM_DB_PATH"] == expected
...
def test_resolve_db_path_db_overrides_project_dir(
    ...
    assert os.environ["GRAPHMEM_DB_PATH"] == str(explicit.resolve())


def test_resolve_db_path_neither(monkeypatch: pytest.MonkeyPatch) -> None:
    """Neither --db nor --project-dir leaves GRAPHMEM_DB_PATH untouched."""
    from graph_mem.cli.main import _resolve_db_path

    monkeypatch.delenv("GRAPHMEM_DB_PATH", raising=False)
    _resolve_db_path()
    assert "GRAPHMEM_DB_PATH" not in os.environ
...
def test_resolve_db_path_preserves_existing_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("GRAPHMEM_DB_PATH", "/existing/db.sqlite")
    _resolve_db_path()
    assert os.environ["GRAPHMEM_DB_PATH"] == "/existing/db.sqlite"
